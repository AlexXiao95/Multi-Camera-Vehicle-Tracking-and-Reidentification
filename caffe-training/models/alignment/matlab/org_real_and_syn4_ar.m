...
savDir ='/home/ljyang/work/data/real_and_syn4_ar/';
train_filename = ['../lists/transfer_train_ar'];
% test_filename = ['../lists/transfer_test'];
val_filename= ['../lists/transfer_val_ar'];
...
for i=1:len
    i
    if model_3d_info(i).is_test==0 %for train
        
       
        param_3d = model_3d_info(i).shape_param;
        model_id = model_3d_info(i).model_id;
...
%                     imshow(im_crop);pause;
        ...
        model_name = model_3d_info(i).model_name;
        pose = dlmread([synDir,model_name,'/',model_name,...
                '_pose_info.txt']);
            
        for im_id = 1:syn_im
            im_path = [synDir,model_name,'/',model_name,...
...
%             im_crop = im_syn(bbox_pad(2):bbox_pad(4),bbox_pad(1):bbox_pad(3),:);
              im_crop = bbox_pad_crop(im_syn,bbox,pad);
             im_crop = im_crop - 0.57;
            if size(im_crop,3)==1
