...
model = 'alexnet_domain2_s_p_f';
model_m = [model,'_mirror'];
...
for i=1:5
    ...
    f_m = load([dataDir,model_m,'_',attr{i}]);
    preds_m = double(f_m.feats(1:sp,:));
    if attr_type(i)==0
        [v,pred_view] = max(preds,[],2);
        [v_m,pred_view_m] = max(preds_m,[],2);
    end
    if i==2
        if v>v_m
            pred_pose(:,1) = (pred_view-1) * h_space;
        else
            pred_pose(:,1) = 360 - (pred_view_m-1)*h_space;
        end
    elseif i==3
        if v>v_m
            pred_pose(:,2) = (pred_view-1) * v_space;
        else
            pred_pose(:,2) = (pred_view_m-1) *v_space;
        end
    elseif i==4
        if v>v_m
            pred_pose(:,3) = (pred_view-1) * r_space - r_max;
        else
            pred_pose(:,3) = (1-pred_view_m)*r_space + r_max;
        end
    elseif i==1
        pred_shape = (preds+preds_m)/2;
    elseif i==5
        pred_focal = (preds+preds_m)/2;
    end
end
